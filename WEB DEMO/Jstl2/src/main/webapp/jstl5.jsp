...
<body>
	...
	<hr/>
	<c:set  var='data1' value="sachin,saurav,dhoni,kohli"/>
	<c:set var="names" value='${fn:split(data1,",")}'/>
	<h1 style="color:green; text-align: center;">
		Result of split method is <br/>	
		<c:forEach var='name' items="${names}">
			${name}<br/>
		</c:forEach>
		<h1> ${fn:length(data1)}</h1>
			
	</h1>
		<hr/>
		
		<c:set var='chutiya' value='${param}'> </c:set>
		<c:out value="${chutiya}"></c:out>
			<h1> ${fn:length(chutiya)}</h1>
			<c:set var="nam" value='${fn:split(chutiya,"&")}'/>
		<h1 style="color:green; text-align: center;">
		Result of split method is <br/>	
		<c:forEach var='namesss' items="${nam}">
			${namesss}<br/>
		</c:forEach>
		<h1> ${fn:length(chutiya)}</h1>
		</h1>
